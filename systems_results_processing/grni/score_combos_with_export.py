...
def score_grn(input_csv, boolean_csv, active_tfs):
    ...
    def determine_state(node, node_states):
        ...
        active_activations = [edge_data['weight'] > 0 for u, v, edge_data in parent_edges if node_states[u] == 'active']
        active_inhibitions = [edge_data['weight'] < 0 for u, v, edge_data in parent_edges if node_states[u] == 'active']
    ...
    def dfs_update_states(active_tfs, node_states):
        ...
        while stack:
            ...
            if node not in visited:
                ...
                stack.extend([v for u, v in G.out_edges(node)])
...
if __name__ == '__main__':
    ...
    if os.path.exists(grni_csv) and os.path.exists(boolean_csv):
        ...
        for combination in tf_combinations:
            score, node_states = score_grn(grni_csv, boolean_csv, combination)
            results.append((combination, score))
            
            
            active_genes = [gene for gene, state in node_states.items() if state == 'active']
            inactive_genes = [gene for gene, state in node_states.items() if state == 'inactive']
            genes_affected[';'.join(combination)] = {
                'active': active_genes,
                'inactive': inactive_genes
            }
            
            counter += 1
            if counter % update_interval == 0 or counter == total_combinations:
                print(f"Processed {counter}/{total_combinations} combinations "
                      f"({(counter/total_combinations)*100:.2f}%)")
        ...
        genes_output_file = output_dir / f"Affected_Genes_{Path(grni_csv).stem}.csv"
        genes_rows = []
        for combo, _ in results:
            # Convert the tuple to string to match how we stored it
            combo_str = ';'.join(combo)
            affected = genes_affected[combo_str]
            genes_rows.append({
                'Combination': combo_str,
                'Active_Genes': '|'.join(affected['active']),
                'Inactive_Genes': '|'.join(affected['inactive'])
            })
        genes_df = pd.DataFrame(genes_rows)
        genes_df.to_csv(genes_output_file, index=False)
        
        print(f"Saved ranked combinations to {output_file}")
        print(f"Saved affected genes to {genes_output_file}")
    else:
